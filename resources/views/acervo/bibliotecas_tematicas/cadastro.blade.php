...
@section('tituloPagina')
    ...
    Registrar Biblioteca Temática <small>{{$equipamento->nome}}</small>
@endsection


@section('conteudo')

    <form method="POST" action="{{ route('bibliotecas.gravar', $equipamento->id) }}" autocomplete="off">
        {{ csrf_field() }}
        ...
        <div class="row">
            <div class="form-group col-md-offset-3 col-md-3">
                ...
                <input type="number" class="form-control" name="acervo" id="acervo"
                       value="{{old('acervo') }}" onblur="calcular()" onkeyup="calcular()">
            </div>
            <div class="form-group col-md-3">
                ...
                <input type="number" class="form-control" name="frequenciaSecao" id="frequenciaSecao"
                       value="{{ old('frequenciaSecao') }}" onblur="calcular()" onkeyup="calcular()">
            </div>
        </div>
        ...
        <div class="row">
            <div class="form-group col-md-offset-3 col-md-3">
                ...
                <input type="number" class="form-control" name="consulta" id="consulta"
                       value="{{ old('consulta') }}" onblur="calcular()" onkeyup="calcular()">
            </div>
            <div class="form-group col-md-3">
                <label for="revista">Empréstimo</label>
                <input type="number" class="form-control" name="emprestimo" id="emprestimo"
                       value="{{ old('emprestimo') }}" onblur="calcular()" onkeyup="calcular()">
            </div>
        </div>
