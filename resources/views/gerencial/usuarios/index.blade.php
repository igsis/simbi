...
@section('conteudo')
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
                                    ...
                                    <td>
                                        ...
                                        @if($user->publicado == 1)
                                            <form method="POST" action="{{ route('usuarios.destroy', $user->id) }}"
                                                ...
                                                <button class="btn btn-danger" data-footer="Desativar" type="button"
                                                        data-toggle="modal" data-target="#confirmDelete"
                                                        data-title="Desativar {{$user->funcionario->nome}}?"
                                                        data-message='Desejar realmente desativar este usuário?'><i
                                                            class="glyphicon glyphicon-trash"></i> Desativar
                                                </button>
                                            </form>
                                        @else
                                            <form method="POST" action="{{ route('ativar.user') }}"
                                                  style="display: inline;">
                                                {{ csrf_field() }}
                                                <input type="hidden" name="type" value="{{ $type }}">
                                                ...
                                                <button class="btn btn-success" type="button" data-toggle="modal"
                                                        ...
                                                        data-title="Ativar {{$user->funcionario->nome}}?"
                                                        data-message='Desejar realmente ativar este usuário?'
            ...
            <div class="modal fade" id="vinculaEquipamento" role="dialog" aria-labelledby="vinculaEquipamentoLabel"
                 aria-hidden="true">
                            ...
                            <form id="formVincula" method="POST" {{--action via javascript--}}>
                                ...
                                @foreach($equipamentos as $equipamento)
                                    <div class="checkbox-inline">
                                        <label><input type="checkbox" name="equipamento[]"
                                                      value="{{$equipamento->id}}">{{$equipamento->nome}}</label>
                                    </div>
                            @endforeach
                        </div>
