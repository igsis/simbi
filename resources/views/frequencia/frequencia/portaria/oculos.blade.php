...
@section('titulo','Público de Óculos)
        ...
        <section class="content">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Lista de público de Óculos</h3>
                </div>
                <div class="box-body">
                    <table id="tabela1" class="table table-bordered table-striped">
                        <thead>
                        @if ($equipamento->oculos->count() != 0)
                            <tr>
                                <th>Data</th>
                                <th>Dia da Semana</th>
                                <th>Total</th>
                        ...
                        <tbody>
                            @foreach ($equipamento->oculos as $frequencia)
                                <tr align="center">
    ...
    <div class="modal modal-danger fade" id="apagarModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('frequencia.portaria.destroyOculos')}}" method="post">
                    {{ csrf_field() }}
...
@section('scripts_adicionais')
    ...
    <script>
        function preencherCampos(data, quantidade, idPublico) {
            limparCampos();
            let formulario = document.querySelector('#formOculos');
            formulario.action = '{{route('frequencia.portaria.updateOculos')}}';
            formulario.setAttribute('method', 'POST')
            document.querySelector('#quantidade').value = quantidade;
            document.querySelector('#calendarioOculos').value = data;
